...
class GroupedQueryAttention(nn.Module):
    ...
    def forward(
        ...
        # KATA(gqa): the attention forward pass. Contract:
        # 1. Derive past_length from past_key_value (sequence axis is -2).
        # 2. Project and reshape via self._shape: Q with num_heads, K/V with
        #    num_kv_heads. Apply self.rope (when set) with offset=past_length.
        # 3. Concatenate the cached K/V along the sequence axis, then capture
        #    `present` as the compact [B, H_kv, T, D] cache.
        # 4. Expansion policy: SDPA's enable_gqa only on CUDA with
        #    query_groups > 1; elsewhere repeat_interleave K/V across
        #    query_groups on the head dim; MHA needs neither.
        # 5. Masking: empty prefix -> attention_mask=None with
        #    is_causal=(query_length > 1). Non-empty prefix -> boolean
        #    [1, 1, T_q, T_k] mask from absolute positions
        #    (key_pos <= past_length + query_pos) with is_causal=False.
        #    Upper-left-aligned is_causal=True here is the documented
        #    cached-logits bug.
        # 6. scaled_dot_product_attention with dropout_p only when training;
        #    transpose/reshape back to [B, T, d_model]; out_proj + residual
        #    dropout.
        # 7. Return (output, present) when use_cache else zero-length cache
        #    tensors (key[:, :, :0], value[:, :, :0]).
        raise NotImplementedError("KATA(gqa): implement the attention forward pass")
