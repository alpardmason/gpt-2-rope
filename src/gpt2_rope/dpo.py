...
def sequence_logprobs(model: GPT, input_ids: Tensor, labels: Tensor) -> Tensor:
    ...
    token_logprobs = log_probs.gather(-1, safe_targets.unsqueeze(-1)).squeeze(-1)
    return (token_logprobs * supervised).sum(dim=-1)
...
def dpo_loss(
    ...
    with torch.no_grad():
        metrics = {
            "dpo/reward_margin": float(margins.mean()),
            "dpo/reward_accuracy": float((margins > 0).float().mean()),
            "dpo/chosen_reward": float(chosen_rewards.mean()),
            "dpo/rejected_reward": float(rejected_rewards.mean()),
        }
    return loss, metrics
